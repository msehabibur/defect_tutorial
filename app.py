...
with tab_basic:
    ...
    with col1:
        ...
        st.info(
            f"""
            **Key insight** The slope of the line equals the charge state **q = {q}**.
...
with tab_atomistic:
    ...
    **Understand how atoms relax around a defect during DFT geometry optimization**
    ...
    if play_animation:
        ...
        # Static mode (not animating) - Fully Expanded to Match Your Request
        col1, col2 = st.columns([1.5, 1])
        ...
        with col2:
            ...
            max_forces = []
            for s in range(21):
                _, _, forces_temp, _ = generate_lattice(lattice_size, defect_type, s, strength=relaxation_strength)
                max_force = np.max([np.linalg.norm(f) for f in forces_temp]) if len(forces_temp) > 0 else 0
                max_forces.append(max_force)
